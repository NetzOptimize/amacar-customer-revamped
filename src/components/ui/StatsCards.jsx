...
const StatsCards = ({ 
  ...
  if (loading) {
    return (
      <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${Math.min(visibleCards.length, 5)} gap-3 sm:gap-4 md:gap-6 ${className}`}>
          ...
          <div key={card.key} className="card p-3 sm:p-4 md:p-6 animate-pulse">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4">
              ...
              <div className="text-left sm:text-right">
                <div className="h-5 sm:h-6 md:h-8 bg-gray-200 rounded w-10 sm:w-12 md:w-16 mb-1 sm:mb-2"></div>
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${Math.min(visibleCards.length, 5)} gap-3 sm:gap-4 md:gap-6 ${className}`}
    >
      {visibleCards.map((card) => {
        ...
        return (
          <motion.div key={card.key} variants={itemVariants} className="card p-3 sm:p-4 md:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4">
              ...
              <div className="text-left sm:text-right">
                <div className="text-base sm:text-lg md:text-2xl font-bold text-neutral-800">
                  {card.isCurrency ? (
                    <CountUp 
                      end={card.value} 
                      duration={1} 
